...
const ApiKeysPage = () => {
    ...
    const handleRegenerate = async () => {
        if (!window.confirm("Are you sure? This will immediately invalidate your old API key.")) return;
    ...
    const getAuthHeader = () => showKey && apiKey !== 'Loading...' && apiKey !== 'No key generated yet' ? apiKey : 'your_api_key_here';
  ...
  -F 'payload={"motion":{"style":"dolly","amplitude":0.2},"render":{"duration":5},"plan":"free"}'`;
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-white tracking-wide">API Keys</h1>
                <p className="text-gray-400 mt-2 text-sm md:text-base">Manage your API keys to authenticate requests to the DepthFlow engine.</p>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div className="bg-[#0b081a]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-lg">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-bold text-white">Integration Quick Start</h2>
                    </div>
                    
                    <div className="bg-black/80 border border-purple-500/20 p-5 rounded-xl overflow-x-auto relative group">
                        <button
...
<span className="text-pink-400">curl</span> -X POST <span className="text-green-400">"{API_BASE_URL}/ai/depthflow/generate-3d"</span> \
  ...
  -F <span className="text-blue-300">"file=@/path/to/image.jpg"</span> \
  -F <span className="text-blue-300">'payload={'{'}"motion":{'{'}"style":"dolly"{'}'},"render":{'{'}"duration":5{'}'}{'}'}'</span>
                        </pre>
                    </div>
                </div>
            </div>
