...
// --- API FAQ DATA ---
const apiFaqs = [
  { question: "How does API work?", answer: "Depthflow API uses a simple credit-based system. Each API request consumes credits depending on the render type, duration, and quality. This allows you to scale usage based on your needs without fixed limitations." },
  { question: "How many credits are used per request?", answer: "A standard 3D render typically consumes 20 credits. Longer durations or higher-quality outputs may require additional credits." },
  { question: "Can I upgrade or downgrade my API plan anytime?", answer: "Yes, you can upgrade or downgrade your API plan at any time based on your usage. Changes take effect instantly or from the next billing cycle." },
  { question: "What happens if I run out of credits?", answer: "If your credits are exhausted, API requests will stop processing. You can either upgrade your plan or purchase additional credits to continue using the service." },
  { question: "Do unused credits roll over to the next month?", answer: "No, unused credits do not roll over. We recommend choosing a plan that matches your expected usage for maximum value." },
  { question: "Is the API suitable for commercial use?", answer: "Yes, all paid API plans allow commercial usage. You can integrate Depthflow into your apps, tools, or platforms and use the outputs for business purposes." },
  { question: "Is payment secure and available worldwide?", answer: "Yes, all payments are securely processed, and we support global payments in multiple currencies, making it easy to subscribe from anywhere in the world." }
];

// --- API PLANS DATA CONFIGURATION ---
const apiPlans = [
  {
    id: "starter", name: "Starter API", price: "99", period: "/month", credits: "10000",
    features: ["Upto 10K credits", "Standard speed", "Basic support"],
    isCustom: false, isHighlighted: false,
    styles: {
      ...
      headerText: "text-gray-100", featureText: "text-[#9a9aa8]", bullet: "bg-[#5a5a68]",
      buttonActive: "bg-[#9d5cff] hover:bg-[#8d4bef]", buttonDisabled: "bg-[#9d5cff]/50 cursor-not-allowed",
      buttonShadow: "shadow-[0_0_20px_rgba(157,92,255,0.25)]",
    }
  },
  {
    id: "growth", name: "Growth API", price: "249", period: "/month", credits: "30000",
    ...
    styles: {
      ...
      headerText: "text-gray-100", featureText: "text-[#9a9aa8]", bullet: "bg-[#5a5a68]",
      buttonActive: "bg-[#5294ff] hover:bg-[#4183ef]", buttonDisabled: "bg-[#5294ff]/50 cursor-not-allowed",
      buttonShadow: "shadow-[0_0_20px_rgba(82,148,255,0.25)]",
    }
  },
  {
    id: "pro", name: "Pro API", price: "499", period: "/month", credits: "65000",
    features: ["Upto 65K credits", "Fastest processing", "Commercial usage"],
    isCustom: false, isHighlighted: true, 
    styles: {
      ...
      headerText: "text-white", featureText: "text-[#e2e2e8] font-medium", bullet: "bg-[#a45cff] shadow-[0_0_5px_#a45cff]",
  ...
  {
    id: "enterprise", name: "Enterprise", price: "Custom pricing", period: "", credits: null,
    features: ["Limit Less Credits", "Dedicated support"],
    isCustom: true, isHighlighted: false,
    styles: {
      ...
      headerText: "text-gray-100", featureText: "text-[#9a9aa8]", bullet: "bg-[#5a5a68]",
      buttonActive: "border border-gray-500/50 text-[#ceceda] hover:bg-white/5 hover:text-white shadow-[inset_0_0_15px_rgba(255,255,255,0.02)]",
      buttonDisabled: "", buttonShadow: "",
    }
  }
];
...
const FAQSection = () => {
  ...
  return (
    <div className="mt-8 max-w-3xl mx-auto w-full px-4 relative z-10">
      <h2 className="text-2xl md:text-3xl font-bold text-center text-white mb-4 tracking-wide">API Pricing FAQs</h2>
      <p className="text-gray-400 text-center mb-12 text-sm md:text-sm">Everything you need to know about our API pricing and billing.</p>

      <div className="space-y-4">
        {apiFaqs.map((faq, index) => (
          <div key={index} className={`border rounded-xl transition-all duration-300 ${openIndex === index ? 'bg-[#151124] border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.1)]' : 'bg-[#151124]/50 border-white/5 hover:border-white/10'}`}>
            <button onClick={() => setOpenIndex(openIndex === index ? null : index)} className="w-full flex items-center justify-between p-6 text-left focus:outline-none">
              <span className="font-medium text-gray-200 text-base md:text-lg pr-4">{faq.question}</span>
              {openIndex === index ? <Minus className="w-5 h-5 text-purple-400 flex-shrink-0" /> : <Plus className="w-5 h-5 text-gray-500 flex-shrink-0" />}
            </button>
...
const ApiPricing = () => {
  ...
  // --- NEW: Handle Custom Enterprise Contact ---
  const handleContactUs = () => {
    const subjectTemplate = "Inquiry: Custom Enterprise API Plan";
      ...
      <main className="flex-1 relative z-10 w-full px-4 py-8 lg:py-12 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1200px] mx-auto w-full pb-10">
          {apiPlans.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`w-full bg-[#151124]/80 backdrop-blur-xl border rounded-[28px] p-6 lg:p-7 flex flex-col relative ${plan.styles.wrapper}`}>
                {plan.isHighlighted && (
                ...
                <div className={`rounded-[20px] py-2.5 mb-8 text-center ${plan.styles.headerBadge}`}>
                  <h3 className={`text-[15px] font-semibold tracking-wide ${plan.styles.headerText}`}>{plan.name}</h3>
                </div>

                <div className={`mb-6 flex ${plan.isCustom ? 'items-center h-[50px]' : 'items-baseline'} ${zIndexClass}`}>
                  {plan.isCustom ? (
                    <span className="text-[26px] xl:text-[28px] font-bold text-white tracking-tight leading-none" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      {plan.price}
                    </span>
                  ) : (
                    <>
                      <span className="text-[38px] xl:text-[42px] font-bold text-white tracking-tight" style={{ fontFamily: "'DM Sans', sans-serif" }}>${plan.price}</span>
                ...
                {plan.isCustom ? (
                  // --- NEW: Changed from <a> to <button> ---
                  <button 
                    onClick={handleContactUs}
                    className={`w-full py-3.5 rounded-2xl text-[15px] font-medium transition-colors mt-auto block text-center ${plan.styles.buttonActive}`}
                  >
                    Contact Us
                  </button>
                ) : (
