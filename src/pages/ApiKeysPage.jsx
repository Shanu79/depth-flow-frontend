                  ...
                  <div>
                    ...
                    <div className="bg-black/80 border border-purple-500/20 p-5 rounded-xl shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] overflow-x-auto relative group">
                      <button 
                        ...
                        className="absolute top-3 right-3 p-2 bg-white/10 hover:bg-white/20 rounded opacity-0 group-hover:opacity-100 transition-opacity"
                      >
...
<span className="text-pink-400">curl</span> -X POST <span className="text-green-400">"{API_BASE_URL}/ai/depthflow/generate-3d"</span> \
  -H <span className="text-yellow-300">"X-API-Key: {showKey && apiKey !== 'Loading...' && apiKey !== 'No key generated yet' ? apiKey : 'YOUR_API_KEY'}"</span> \
  ...
  -F <span className="text-blue-300">'payload={'{'}"motion":{'{'}"style":"dolly","amplitude":0.2,"speed":1.0...{'}'}, "render":{'{'}...{'}'}, "effects":{'{'}...{'}'}, "plan":"free"{'}'}'</span>
                      </pre>
                    </div>
                  </div>
